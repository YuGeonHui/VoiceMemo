            ...
            .navigationDestination(for: PathType.self) { pathType in
                    ...
                    case let .memoview(isCreateMode, memo):
                        MomoView(
                            ...
                            : .init(memo: memo ?? .init(title: "", content: "", date: .now)), isCreateMode: isCreateMode)
                            .navigationBarBackButtonHidden()
                            .environmentObject(memoListViewModel)
                    }
                }
    ...
    fileprivate var body: some View {
        ...
        TabView(selection: $selectedIndex) {
            ForEach(Array(onboardingViewModel.onboardingContents.enumerated()), id: \.element) { index, onboardingContent in
                OnboardingCellView(onboardingContents: onboardingContent)
                    .tag(index)
            }
        }
...
private struct StartBtnView: View {
    ...
    fileprivate var body: some View {
        ...
        Button(action: { pathModel.paths.append(.homeView) } , label: {
